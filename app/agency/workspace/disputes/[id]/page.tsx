...
export default async function WorkspaceDisputeDetailPage({ params }: PageProps) {
  ...
  // Permission check: dispute must be for a contract in this workspace or a visible job
  const isWorkspaceContract = contract.workspace_id === context.workspace.id;
  let isAllowed = isWorkspaceContract;

  if (!isAllowed && contract.job_id) {
    ...
    isAllowed = job?.workspace_id === context.workspace.id &&
      (context.isOwner || job.created_by_user_id === context.userId);
  }
