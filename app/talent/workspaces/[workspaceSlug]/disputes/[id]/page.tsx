...
export const metadata: Metadata = { title: "Detalhe da Disputa — BrisaHub" };
...
const RESOLUTION_LABEL: Record<string, string> = {
  release: "Valor liberado para você",
  refund:  "Valor reembolsado à agencia",
  split:   "Divisao parcial",
  close:   "Encerrada sem movimentacao",
};

export default async function WorkspaceTalentDisputeDetailPage({ params }: Props) {
  ...
  return (
    <div className="max-w-4xl space-y-6 px-4 pb-10 sm:px-6">
      <div>
        <Link href={`/talent/workspaces/${workspaceSlug}/disputes`} className="text-[12px] font-semibold text-teal-700 hover:text-teal-800">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <section className="rounded-3xl border border-zinc-100 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 space-y-3">
            ...
            <div className="flex justify-between text-[13px]">
              <span className="text-zinc-500">Você receberia</span>
              <span className="font-semibold text-zinc-900">{brl(amounts.net)}</span>
            </div>
            <div className="flex justify-between text-[13px]">
              <span className="text-zinc-500">Valor bruto</span>
              <span className="font-medium text-zinc-700">{brl(amounts.gross)}</span>
            </div>
            <div className="h-px bg-zinc-100" />
            <div className="flex justify-between text-[13px]">
          ...
          {isResolved ? (
              ...
              <div className="mt-5 space-y-3">
                ...
                {(dispute.talent_amount == null || Number(dispute.talent_amount) === 0) && dispute.resolution_action === "refund" ? (
                  <div className="rounded-2xl bg-zinc-100 p-4">
                    <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-500">Valor reembolsado à agencia</p>
                    <p className="mt-1 text-[13px] text-zinc-600">O valor foi devolvido à agência. Nenhum crédito foi gerado para você.</p>
                  </div>
                ) : null}
            ...
            <>
              ...
              <div className="mt-4 rounded-2xl bg-amber-50 p-4">
                ...
                <p className="mt-1 text-[22px] font-semibold text-amber-900">{brl(amounts.net)}</p>
                <p className="mt-1 text-[12px] text-amber-700">Seu valor está aguardando a resolução da disputa.</p>
              </div>
            </>
          )}
        </section>
