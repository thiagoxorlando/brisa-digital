...
export const metadata: Metadata = { title: "User Profile — Admin — BrisaHub" };
...
export default async function AdminUserProfilePage({ params }: Props) {
  ...
  const [
    ...
    supabase.from("contracts").select("payment_amount, commission_amount, net_amount, status").eq("talent_id", id).in("status", ["signed", "confirmed", "paid"]).is("deleted_at", null),
    supabase.from("contracts").select("payment_amount, commission_amount, net_amount, status").eq("agency_id", id).in("status", ["signed", "confirmed", "paid"]).is("deleted_at", null),
