...
CREATE TABLE IF NOT EXISTS "public"."contract_disputes" (
    ...
    CONSTRAINT "contract_disputes_resolved_check" CHECK ((("status" <> ALL (ARRAY['resolved_refund'::"text", 'resolved_release'::"text", 'closed'::"text"])) OR (("resolved_at" IS NOT NULL) AND ("resolved_by_user_id" IS NOT NULL)))),
    CONSTRAINT "contract_disputes_status_check" CHECK (("status" = ANY (ARRAY['open'::"text", 'under_review'::"text", 'resolved_refund'::"text", 'resolved_release'::"text", 'closed'::"text"])))
);
